...
class DBLogging():
    ...
    def __init__(self, *args, **kwargs):
        ...
        if os.path.exists(f'{pathlib.Path().absolute()}/GEDI/DB_LOGS') is False:
            os.makedirs(f'{pathlib.Path().absolute()}/GEDI/DB_LOGS')

        logfilename = f"{pathlib.Path().absolute()}/GEDI/DB_LOGS/{datetime.datetime.now().strftime('%Y%m%d')}.log"
        l = 1
        if os.path.exists(logfilename):
            lines = open(logfilename, mode='r')
            l = len(lines.readlines()) + 1
